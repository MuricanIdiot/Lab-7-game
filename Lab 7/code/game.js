...
//var myVector3 = myVector1.plus(myVector2);
...
// this.wrap corresponds to a div created with class of "game"
  this.wrap = parent.appendChild(elt("div", "Wgame"));
  this.level = level;
...
Player.prototype.moveX = function(step, level, keys) {
  ...
  //find if there is an new obsticle at the newPos
  var obstacle = level.obstacleAt(newPos, this.size);

  // Move if theres not a wall there
if (obstacle != "wall")
  this.pos = newPos;
...
// Accelerate player downwards always
  ...
  //the Floor is also an obstacle-- only allow player to jump
  //if they are touching an obstacle
  if (obstacle) {
...
//lava, then set the vector as a spawn point this could also work with portals
    if (obstacle == 'lava')
  	  this.pos = new Vector(6,20);
};
